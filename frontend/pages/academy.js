...
// Módulos, fases y lecciones de cada módulo: en academy_modulos.js, que
// comparte con el Dashboard.
...
export async function render(container) {
    ...
    // Igual con `?modulo=12`, que abre la página del módulo (donde está su
    // quiz): lo usa la tarjeta del Dashboard cuando lo que falta es el quiz.
    const pedido = new URLSearchParams(window.location.search);
    const leccionPedida = pedido.get('leccion');
    const moduloPedido  = pedido.get('modulo');
    injectStyles();
    ...
    const progreso = cargarProgreso().then(() => actualizarBarras(container));
    ...
    } else if (moduloPedido && /^\d{1,3}$/.test(moduloPedido)) {
        const m = MODULES.find(x => x.id === Number(moduloPedido));
        if (m) {
            history.replaceState(history.state, '', '/academy');
            // La página del módulo marca las lecciones leídas y el quiz: sin
            // esperar al progreso saldría todo como pendiente.
            await progreso;
            await renderModuleDetail(container, m);
        }
    }
}
