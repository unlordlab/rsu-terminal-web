...
(FastAPI). Regenerado 22/07/2026 (sesion 19) desde la tabla real de
constituyentes de Wikipedia (https://en.wikipedia.org/wiki/List_of_S%26P_500_companies,
activamente mantenida, se actualiza en cada reconstitucion del indice).

La version anterior (20/07/2026, "unificacion" de 3 copias identicas en
scanner_universe.py/rsrw_scan.py/rsrw_service.py) cerro la duplicacion de
codigo pero partio de una fuente ya desactualizada: verificado contra
Wikipedia que le faltaban 73 miembros reales (BA, BX, CEG, MDT, TT, TROW,
Berkshire Hathaway entero...) y tenia 83 que ya no son miembros (SGEN
adquirida por Pfizer 2023, US Steel/X adquirida por Nippon 2025, WBA,
BBWI, DISH...) -- ~30% de las 513 entradas estaban mal. Ademas, BF.B
estaba guardado con notacion de punto, que yfinance no reconoce
(yf.Ticker("BF.B") devuelve vacio, yf.Ticker("BF-B") trae datos reales) --
Brown-Forman llevaba tiempo cayendose en silencio de cualquier scan que
usara este universo.

Notacion de tickers con clase de accion: BRK.B->BRK-B, BF.B->BF-B (los
unicos 2 casos en todo el S&P 500 real) -- yfinance exige guion, no punto.

Para actualizar el universo en el futuro (cambios de composicion del
S&P 500, ~20/año): volver a correr el mismo proceso contra la tabla de
Wikipedia (descargar con User-Agent explicito, Wikipedia devuelve 403 sin
uno; parsear la tabla id="constituents"; convertir BRK.B/BF.B a notacion
con guion) y sustituir el diccionario de abajo entero. Editar UNICAMENTE
este archivo -- los consumidores (canslim_service.py, options_service.py,
scanner_universe.py, rsrw_scan.py, rsrw_service.py, sector_medians.py) lo
leen desde aqui.
